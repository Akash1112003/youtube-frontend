...
function Comment({ comment, refresh }) {
    ...
    const handleDelete = async () => {
        try {
            const res = await axios.delete(`https://youtube-backend-eo4h.onrender.com/api/comment/delete/${comment._id}`,{ withCredentials: true });
            if (res.data.success) {
    ...
    async function handleEdit() {
        try {
            ...
            const res = await axios.put(`https://youtube-backend-eo4h.onrender.com/api/comment/edit/${videoId}`, { text: commentText, commentId: comment._id }, { withCredentials: true });
            if (res.data.success) {
                setIsEditing(false);
            }
        } catch (error) {
    ...
    return (
        <div className="flex space-x-4 mb-4">
            <Avatar className="h-8 w-8">
                <AvatarImage src={`https://youtube-backend-eo4h.onrender.com/api/uploads/${comment.user.profilePicture}`} />
